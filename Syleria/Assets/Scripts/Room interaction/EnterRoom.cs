...
public class EnterRoom : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < m_aSpawnPoints.Length; i++) // Loops through Spawn points
        {
            if(m_aSpawnPoints[i].GetComponent<SpawnPoints>().m_bDone == true && m_enemy.Length <= 0 && m_bIsBoss)
            {
            ...
            else if (m_aSpawnPoints[i].GetComponent<SpawnPoints>().m_bDone == true && m_enemy.Length <= 0 && !m_bIsBoss) // Checks if rooms are done and there are no enemies
            {
